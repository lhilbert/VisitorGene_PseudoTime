...
% sourceDirectory = './ExtractedStacks_2/Visitor/**/';
% sourceDirectory = './ExtractedStacks_2/Inactive/**/';
% sourceDirectory = './ExtractedStacks_2/Independent/**/';

% sourceDirectory = './ExtractedStacks_2/**/';
sourceDirectory = './ExtractedStacks/HighRepeatNumber/**/';
...
% 		waitforbuttonpress
		...
		if OP_comps.NumObjects>0 && Cluster_comps.NumObjects>0
			...
			for cl = 1:numel(Cluster_Volume_array)
				...
				Cluster_Mask_cell{cl} = thisImage;
...
% 	set(gca,'XLim',[-6,3],'YLim',[-2,8])
	...
	angles = -atan2(trafo_scores(:,2),trafo_scores(:,1))./pi; % in radians
	angles = mod(angles,2);
	...
	plot(shiftVals([1,end]),[0,0],'k-','LineWidth',1)
	hold on
...
for cc = 1:numConds
	...
	numPlots = numel(sorted_central_slices{cc});
	for pp = 1:numPlots
		subplot(ceil(sqrt(numPlots)),ceil(sqrt(numPlots)),pp)
		imagesc(sorted_central_slices{cc}{pp})
		axis equal tight
		title(sprintf('Image %d',pp))
	end
	colormap(gray)
