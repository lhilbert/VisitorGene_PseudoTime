...
sourceDirectory = './ExtractedStacks_Sphere/Cond_8/**/';
% sourceDirectory = './klf2b_Sphere/';
...
% 	waitforbuttonpress
	...
	OP_displacement = cell(1,numNuclei);
...
% 		waitforbuttonpress
		...
		if OP_comps.NumObjects>0 && Cluster_comps.NumObjects>0
			...
			% Calculate displacement vector for every cluster-gene pair
			OP_displ_array = ...
				(OP_props.Centroid-Cluster_props.Centroid) .* ...
			...
			OP_displacement{nn} = OP_displ_array;
...
% 						ceil(boundingBox(3)+0.5+0.5.*(boundingBox(6)-1));
	...
	OP_displCell{ff} = vertcat(OP_displacement{:});
...
sortedOPdisplCell = cell(1,numConds);

sortedPixelSize = cell(1,numConds);
sortedZStep = cell(1,numConds);
...
for cc = 1:numConds
	...
	OP_displs = vertcat(OP_displCell{condInds==uniqueCondInds(cc)});
...
sorted_displ_vectors = cell(1,numConds);
...
for cc = 1:numConds
	...
	Displ_vectors = sortedOPdisplCell{cc}(inclInds,:);
...
% 	set(gca,'XLim',[-6,3],'YLim',[-2,8])
	...
	sorted_displ_vectors{cc} = Displ_vectors(sortInds,:); 
...
%% --- plot the cluster masks sorted by the OP-cluster distance

num_snapshots = 25;
snapshot_window_edges = linspace(0,1,num_snapshots+1);
...
for cc = 1:numConds
	...
	numImgs = numel(sorted_central_slices{cc});
	ss_coords = (1:numImgs)./numImgs;
	
	OP_maxInts = cellfun(@(img_cell)prctile(img_cell{3}(:),99),...
		sorted_central_slices{cc});
	sliceSizes = cellfun(@(img_cell)numel(img_cell{3}(:)),...
		sorted_central_slices{cc});
	
	imgEdgeLengths = sqrt(sliceSizes).*sortedPixelSize{cc}.*1;
	
	for nn = 1:num_snapshots
		
		in_inds = find(...
			ss_coords>snapshot_window_edges(nn) & ...
			ss_coords<=snapshot_window_edges(nn+1) & ...
			sliceSizes'==max(sliceSizes));
		[~,max_int_ind] = max(OP_maxInts(in_inds));
		max_int_ind = in_inds(max_int_ind);
		this_ss = ss_coords(max_int_ind);
		this_displ = sorted_displ_vectors{cc}(max_int_ind,:);
		
		thisPlot = mod(nn-1-round(num_snapshots),num_snapshots)+1;
		subplot(ceil(sqrt(num_snapshots)),...
			ceil(sqrt(num_snapshots)),thisPlot)
		
		magenta_channel = ...
			(sorted_central_slices{cc}{max_int_ind}{1}-0.4)./2.5;
		green_channel = ...
			(sorted_central_slices{cc}{max_int_ind}{3}-1.05)./0.5;
		
		RGB_img = zeros(size(green_channel,1),size(green_channel,2),3);
		RGB_img(:,:,1) = magenta_channel;
		RGB_img(:,:,2) = green_channel;
		RGB_img(:,:,3) = magenta_channel;
		
% 		RGB_img(:,:,1) = min(1,max(0,magenta_channel))+green_channel;
% 		RGB_img(:,:,2) = min(1,max(0,magenta_channel))-green_channel;green_channel;
% 		RGB_img(:,:,3) = min(1,max(0,magenta_channel))-green_channel;magenta_channel;
		
% 		RGB_img(:,:,1) = magenta_channel;
% 		RGB_img(:,:,2) = green_channel;
% 		RGB_img(:,:,3) = green_channel;
		
		
		image([0,imgEdgeLengths(max_int_ind)],...
			[0,imgEdgeLengths(max_int_ind)],RGB_img)
		hold on
		plot(imgEdgeLengths(max_int_ind).*0.5...
			+[0,this_displ(1)],...
			imgEdgeLengths(max_int_ind).*0.5...
			+[0,this_displ(2)],'wo-','MarkerFaceColor',[0,0,0])
		plot(imgEdgeLengths(max_int_ind).*0.5...
			+[this_displ(1)],...
			imgEdgeLengths(max_int_ind).*0.5...
			+[this_displ(2)],'ko','MarkerFaceColor',[1,1,1])
		
		if nn == 1
			
			hold on
			plot([0.2,1.2],[0.2,0.2],'w-','LineWidth',3)
			text(0.7,0.6,'1 \mum','Color',[1,1,1],....
		...
		axis equal tight
		title(sprintf('s=%3.3f',this_ss),'FontWeight','normal')
